...
model {
  ...
  sigma ~ lognormal(log(1.0 / 8.5), 0.5);   // Prior mean: 5-day incubation period
  gamma ~ lognormal(log(1.0/9), 0.5);   // Prior mean: 7-day infectious period
...
generated quantities {
  ...
  for (i in 1:T)
    predicted_cases[i] = neg_binomial_2_rng(reporting_rate*incidence[i]+0.00001, phi);
