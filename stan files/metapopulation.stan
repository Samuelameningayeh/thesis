...
data {
  ...
  int<lower=1> n_weeks;             
  vector[4*N_patches] y0;           // Initial states for all patches [S1,E1,I1,R1,...]
  real t0;                         
...
transformed parameters {
  array[n_weeks] vector[4*N_patches] y;     // States over time
...
model {
  ...
  for (p in 1:N_patches)
    for (j in 1:N_patches)
      beta_mat[p, j] ~ lognormal(log(beta_data[p, j]), 0.3);

  sigma ~ lognormal(log(sigma_data), 0.3);
  gamma ~ lognormal(log(gamma_data), 0.3);
